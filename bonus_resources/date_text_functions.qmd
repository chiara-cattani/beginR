...
problematic_dates <- tibble(
  ...
  BIRTH_DATE = c(
    ...
    as.Date("1985-12-31"),
    as.Date("1990-02-29"),  # Leap year - valid
    as.Date("1992-08-15"),
...
utc_times <- multi_site_data %>%
  mutate(
    # Parse local time with timezone
    LOCAL_DATETIME = map2_dbl(LOCAL_TIME, TIMEZONE, 
                              ~as.numeric(ymd_hms(.x, tz = .y))),
    LOCAL_DATETIME = as_datetime(LOCAL_DATETIME, tz = "UTC"),
    
    # Alternative method using with_tz
    LOCAL_PARSED = ymd_hms(LOCAL_TIME),
    
    # Format for CDISC submission (ISO 8601 UTC)
    UTC_ISO = format(LOCAL_DATETIME, "%Y-%m-%dT%H:%M:%S", tz = "UTC"),
    
    # Study day based on UTC
    STUDY_START_UTC = ymd("2024-03-15", tz = "UTC"),
    STUDY_DAY = as.numeric(date(LOCAL_DATETIME) - date(STUDY_START_UTC)) + 1
  )
